...
class Direct_mapped_cache:
    def __init__(self, ui):
        ...
        # Cache parameters
        self.cache_size = 0
        # internal field renamed to line_size
        self.line_size = 0
    ...
    def direct_mapped(self):
        ...
        self.line_size = max(1, int(self.ui.line_size.get()))
        self.address_width = max(1, int(self.ui.address_width.get()))
        self.replacement_policy = self.ui.replacement_policy.get()

        # Calculate cache parameters
        self.num_blocks = self.cache_size // self.line_size
        self.block_offset_bits = (self.line_size - 1).bit_length() if self.line_size > 1 else 0
        ...
        print(f"  Line size: {self.line_size}")
        ...
        for i in range(self.num_blocks):
            ...
            cache_line = [str(i), "0", "-"] + ["-"] * self.line_size
            self.cache_contents.append(cache_line)

        # Initialize core cache for compatibility
        write_policy = self.ui.write_hit_policy.get()
        # pass the new parameter name 'line_size' to core Cache
        self.cache = Cache(num_blocks=self.num_blocks, line_size=self.line_size, associativity=1,
